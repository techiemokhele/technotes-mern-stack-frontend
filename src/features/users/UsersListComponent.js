...
const UsersListComponent = () => {
    ...
    if (isSuccess) {
        ...
        const employees = ids.filter(id => entities[id]?.roles.includes('Employee')).length
        const managers = ids.filter(id => entities[id]?.roles.includes('Manager')).length
        const admins = ids.filter(id => entities[id]?.roles.includes('Admin')).length
                        ...
                        <div className='hidden sm:flex-1 sm:flex sm:items-center sm:justify-between'>
                            <div>
                                <p className='text-sm text-orange-500'>
                                    Showing <span className='font-medium text-white'>{indexOfFirstItem + 1}</span> to <span className='font-medium text-white'>{Math.min(indexOfLastItem, totalUsers)}</span> of{' '}
